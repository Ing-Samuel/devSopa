    ...
    <body>
        <div>
            <?php
				...
				if (isset($_POST["save"])) {
					if (($_POST["emailDate"] != "") || ($_POST["address"] != "") || ($_POST["city"] != "")) {
						// Continue with Update.
						$sqlUpdate = "UPDATE pqr SET fecha_modificacion = '".$date."', id_tarea = '".$_POST["task"]."', isp = '".$_POST["isp"]."', prioridad = '".$_POST["priority"]."', activo = ".$_POST["active"].", fecha_correo = '".$_POST["emailDate"]."', direccion = '".$_POST["address"]."', ciudad = '".$_POST["city"]."' WHERE id = ".$id;   
						$resultUpdate = mysqli_query($db, $sqlUpdate);
						//echo $sqlUpdate;
						echo "<script type='text/javascript'>";
							echo "window.location = 'load_pqr.php';";
						echo "</script>";
					} else {
						$error1 = true;
					}
				...
				// Table.
				$sqlTable = "SELECT tareas_pqr.tarea, pqr.prioridad, pqr.activo, pqr.fecha_correo, DATEDIFF(CURRENT_TIMESTAMP, pqr.fecha_correo) as dias, pqr.direccion, pqr.ciudad, pqr_isp.isp 
        ...
        <div id="main">
            <center><h3>UPDATE PQR</h3>
            <br>
			<form id="updatepqr" method="POST" action="update_pqr.php">
						...
						echo "<tr>";
							echo "<td align='center'>";
								echo "<select id='task' name='task' style='width:200x;'>";
									for ($i = 0; $i < count($taskSelect); $i++) {
										if ($taskSelect[$i]["id_tarea"] == $result_table[0]["tarea"])
											echo "<option value='".$taskSelect[$i]["id_tarea"]."' selected>".$taskSelect[$i]["tarea"]."</option>";				
										else
							...
							echo "<td align='center'>";
								...
								for ($i = 0; $i < count($ispSelect); $i++) {
									if ($ispSelect[$i]["isp"] == $result_table[0]["isp"])
										echo "<option value='".$ispSelect[$i]["id_isp"]."' selected>".$ispSelect[$i]["isp"]."</option>";
									else
							...
							echo "<td align='center'>";
								echo "<select id='active' name='active' style='width:80x;'>";
									for ($i = 0; $i < count($activo); $i++) {
										if ($i == $result_table[0]["activo"]) 
											echo "<option value='".$i."' selected>".$activo[$i]."</option>";
										else 
							...
							echo "<td align='center'><input type='text' id='emailDate' name='emailDate' value='".$result_table[0]["fecha_correo"]."' maxlenght='10' onkeypress='return validateDate(event)' style='width:80px'></td>";
							echo "<td align='left'><input type='text' placeholder='Address' id='address' name='address' value='".$result_table[0]["direccion"]."' maxlenght='50' onkeypress='return validateAddress(event)' style='width:200px'></td>";
							echo "<td align='left'><input type='text' placeholder='City' id='city' name='city' value='".$result_table[0]["ciudad"]."' maxlenght='15' onkeypress='return validateCity(event)' style='width:150px'></td>";
						echo "</tr>";
					?>
					</tbody>    
				</table>
        ...
        <script type="text/javascript">
			...
			function selected(selectobj1){
                document.getElementById("updatepqr").submit();
            }		
		</script>
    </body>
</html>