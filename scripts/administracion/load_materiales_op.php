...
<html>
    ...
    <body>
        ...
        <div id="main">
            <center><h3>LOAD FILE - OP MATERIALS</h3>
            <br>
            <form enctype="multipart/form-data" id="load" method="POST" action="load_materiales_op.php">
                ...
                if (isset($_POST["load"])) {
                    if ($_FILES["fileLoad"]["error"]) {    
                        ...
                        if (pathinfo($_FILES['fileLoad']['name'], PATHINFO_EXTENSION) == "csv") {
                            $location = "opMaterials.csv"; 
                            move_uploaded_file($_FILES["fileLoad"]["tmp_name"], $location);
                            $file = @fopen("opMaterials.csv", 'r');
                            if ($file) {
                                // DB Parameters.
								...
								// DB Parameters.
                                $mysql_database = "huawei_qlik";
								
								$db2 = mysqli_connect($mysql_host, $mysql_user, $mysql_password, $mysql_database) or die ("Error durante la conexión a la base de datos");
                                ...
                                while (($line = fgets($file)) !==  FALSE) {
									...
									$sqlInsert = str_replace("materiales_contratos", "bodegas_stock_op", $sqlInsert);
