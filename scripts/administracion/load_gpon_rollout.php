...
<html>
    ...
    <body>
            ...
            <?php
				...
				if (isset($_POST["save"])) {
					if (!($_POST["year"] == "") && !($_POST["homepassed"] == "")) {
						$sqlInsert = "INSERT INTO gpon_rollout (fecha_balance, year, mes, zona, ciudad, tipo, homepassed) 
				...
				// Table.
				//$sqlTable = "SELECT id, year, mes, ciudad, tipo, homepassed FROM gpon_rollout WHERE year = ".substr($date, 0, 4);
				$sqlTable = "SELECT id, year, mes, ciudad, tipo, homepassed FROM gpon_rollout WHERE year = 2016";
        ...
        <div id="main">
            ...
            <br>
			<form id="gpon_rollout" method="POST" action="load_gpon_rollout.php">
							...
							for ($i = 0; $i < count($result_table); $i++) {
								echo "<tr align='center'>";	
									echo "<td>".$result_table[$i]["year"]."</td><td>"
										 ...
										 .$result_table[$i]["ciudad"]."</td><td>"
										 .$result_table[$i]["tipo"]."</td><td>"
										 .$result_table[$i]["homepassed"]."</td><td><a href='update_gpon_rollout.php?id=".$result_table[$i]["id"]."'>Change</a></td>";
								echo "</tr>";    
							}
						}
					?>
        ...
        <script type="text/javascript">
			...
			function selected(selectobj1){
                document.getElementById("gpon_rollout").submit();
            }		
		</script>
    </body>
</html>
