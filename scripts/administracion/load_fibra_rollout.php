    ...
    <body>
        <div>
            <?php
				...
				if (isset($_POST["save"])) {
					if (!($_POST["year"] == "") && !($_POST["clientes_uk"] == "")) {
						$sqlInsert = "INSERT INTO fibra_rollout (fecha_balance, year, mes, zona, ciudad, clientes_uk) 
									  VALUES ('".$date."', ".trim($_POST["year"]).", '".$_POST["month"]."', '', '".$_POST["city"]."', ".trim($_POST["clientes_uk"]).")";
				...
				// Selects.
				//$sqlCities = "SELECT DISTINCT(ciudad) FROM meta_fibra WHERE year = ".substr($date, 0, 4)." ORDER BY ciudad";
				$sqlCities = "SELECT DISTINCT(ciudad) FROM meta_fibra WHERE year = 2016 ORDER BY ciudad";
				...
				// Table.
				//$sqlTable = "SELECT id, year, mes, ciudad, clientes_uk FROM fibra_rollout WHERE year = ".substr($date, 0, 4);
				$sqlTable = "SELECT id, year, mes, ciudad, clientes_uk FROM fibra_rollout WHERE year = 2016";
        ...
        <div id="main">
            <center><h3>F.O ROLLOUT ADMON</h3>
            <br>
			<form id="fibra_rollout" method="POST" action="load_fibra_rollout.php">
					...
					<?php
						if ($activity == "New Entry") {
							...
							echo "<td align='center'>";
								...
								echo "</select>";
							echo "</td>";
							echo "<td align='center'><input type='text' id='clientes_uk' name='clientes_uk' onkeypress='return validateNumber(event)' style='width:50px'></td>";
						} else {
							echo "<thead>";
								echo "<tr>";
									echo "<th>Year</th>";
							...
							for ($i = 0; $i < count($result_table); $i++) {
								echo "<tr align='center'>";	
									echo "<td>".$result_table[$i]["year"]."</td><td>"
										 .$result_table[$i]["mes"]."</td><td>"
										 .$result_table[$i]["ciudad"]."</td><td>"
										 .$result_table[$i]["clientes_uk"]."</td><td><a href='update_fibra_rollout.php?id=".$result_table[$i]["id"]."'>Change</a></td>";
								echo "</tr>";    
							}
						}
					?>
        ...
        <script type="text/javascript">
			...
			function selected(selectobj1){
                document.getElementById("fibra_rollout").submit();
            }		
		</script>
    </body>
</html>
