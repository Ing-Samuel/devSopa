...
<html>
    ...
    <body>
            ...
            <?php
				...
				if (isset($_POST["save"])) {
					if (!($_POST["year"] == "") && !($_POST["homepassed"] == "")) {
						$sqlInsert = "INSERT INTO gpon_etp_rollout (fecha_balance, year, mes, zona, ciudad, tipo, homepassed) 
				...
				// Selects.
				//$sqlCities = "SELECT DISTINCT(ciudad) FROM meta_gpon_etp WHERE year = ".substr($date, 0, 4)." ORDER BY ciudad";
				$sqlCities = "SELECT DISTINCT(ciudad) FROM meta_gpon_etp WHERE year = 2016 ORDER BY ciudad";
				...
				// Table.
				//$sqlTable = "SELECT id, year, mes, ciudad, tipo, homepassed FROM gpon_etp_rollout WHERE year = ".substr($date, 0, 4);
				$sqlTable = "SELECT id, year, mes, ciudad, tipo, homepassed FROM gpon_etp_rollout WHERE year = 2016";
        ...
        <div id="main">
            <center><h3>GPON ROLLOUT ADMON</h3>
            <br>
			<form id="gpon_etp_rollout" method="POST" action="load_gpon_etp_rollout.php">
						...
						if ($activity == "New Entry") {
							...
							/*echo "<td align='center'>";
								echo "<select id='type' name='type' style='width:70px;'>";
									...
									echo "<option value='Mig'>Mig</option>";
								echo "</select>";
							echo "</td>";*/
							...
							for ($i = 0; $i < count($result_table); $i++) {
								echo "<tr align='center'>";	
									echo "<td>".$result_table[$i]["year"]."</td><td>"
										 ...
										 .$result_table[$i]["ciudad"]."</td><td>"
										 //.$result_table[$i]["tipo"]."</td><td>"
										 .$result_table[$i]["homepassed"]."</td><td><a href='update_gpon_etp_rollout.php?id=".$result_table[$i]["id"]."'>Change</a></td>";
								echo "</tr>";    
							}
						}
					?>
        ...
        <script type="text/javascript">
			...
			function selected(selectobj1){
                document.getElementById("gpon_etp_rollout").submit();
            }		
		</script>
    </body>
</html>
