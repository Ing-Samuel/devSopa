...
<html>
    ...
    <body>
            ...
            <?php
				...
				if (isset($_POST["save"])) {
					if (!($_POST["year"] == "") && !($_POST["homepassed"] == "")) {
						$sqlInsert = "INSERT INTO hfcb2b_rollout (fecha_balance, year, mes, zona, ciudad, tipo, homepassed) 
									  VALUES ('".$date."', ".trim($_POST["year"]).", '".$_POST["month"]."', '', '".$_POST["city"]."', '".$_POST["type"]."', ".trim($_POST["homepassed"]).")";
				...
				// Selects.
				//$sqlCities = "SELECT DISTINCT(ciudad) FROM meta_hfcb2b WHERE year = ".substr($date, 0, 4)." ORDER BY ciudad";
				$sqlCities = "SELECT DISTINCT(ciudad) FROM meta_hfcb2b WHERE year = 2016 ORDER BY ciudad";
				...
				// Table.
				//$sqlTable = "SELECT id, year, mes, ciudad, tipo, homepassed FROM hfcb2b_rollout WHERE year = ".substr($date, 0, 4);
				$sqlTable = "SELECT id, year, mes, ciudad, tipo, homepassed FROM hfcb2b_rollout WHERE year = 2016";
        ...
        <div id="main">
            <center><h3>HFCB2B ROLLOUT ADMON</h3>
            <br>
			<form id="hfcb2b_rollout" method="POST" action="load_hfcb2b_rollout.php">
							...
							for ($i = 0; $i < count($result_table); $i++) {
								echo "<tr align='center'>";	
									echo "<td>".$result_table[$i]["year"]."</td><td>"
										 ...
										 .$result_table[$i]["ciudad"]."</td><td>"
										 .$result_table[$i]["tipo"]."</td><td>"
										 .$result_table[$i]["homepassed"]."</td><td><a href='update_hfcb2b_rollout.php?id=".$result_table[$i]["id"]."'>Change</a></td>";
								echo "</tr>";    
							}
						}
					?>
        ...
        <script type="text/javascript">
			...
			function selected(selectobj1){
                document.getElementById("hfcb2b_rollout").submit();
            }		
		</script>
    </body>
</html>
